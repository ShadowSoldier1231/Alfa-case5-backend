...
Легковесный эндпоинт для Health Check. Используется мониторингом (например, Docker) для проверки того, что приложение запущено и база данных доступна.
```bash
...
### Проверка готовности к обработке запросов (`GET`)
В отличие от '/health', может возвращать статус 503 в случае "database":"DOWN".
```bash
...
### Уменьшенная проверка работоспособности (`GET`)
Легковесный эндпоинт для мониторинга статуса приложения.
```bash
